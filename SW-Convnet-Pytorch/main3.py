...
parser.add_argument('--data_test', metavar = 'DIR', default = 'imagenet',
                    help = 'path to dataset')
...
parser.add_argument('-b', '--batch_size', default = 156, type = int,
                    metavar = 'N',
...
def morlet_wavelet(t):
    return torch.exp(1j * 2.5 * t) * torch.exp(-0.5 * t**2)
...
def main_worker(gpu, args, printer):
    ...
    # create model
    if args.pretrained:
        archs=args.arch.split('_')
        print(archs[0])
        # printer.pprint("=> using pre-trained model '{}'".format(args.arch))
        # model = models.__dict__[args.arch](pretrained = True)
        printer.pprint("=> using pre-trained model '{}'".format(archs[0]))
    ...
    #crop_size = 256 if (args.evaluate_shift or args.evaluate_save) else 224    #modify crop_size 2025-10-27
    crop_size = 256
    ...
    #model = torch.load('model.pth')  #load pre-model 2024-10-13
    
    # # compute params and Flops
    # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    # input0=torch.randn(1,3,224,224).to(device)
    # #model=model.module.to(device)
    # #model = model.module.to(torch.device('cpu'))
    # Flops, params = profile(model.module.to(device), inputs=(input0,)) # macs
    # print('Flops: % .4fG'%(Flops / 1e9))# 计算量
    # print('params参数量: % .4fM'% (params / 1e6))
    # model=model.cuda()
...
def train(train_loader, model, criterion, optimizer, epoch, args, printer, start_time):
    ...
    # print(f"ResNet-50总参数量：{total_params} ≈ 2555万")
    # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    # if args.gpu is not None:
    #   input0=torch.randn(10,3,512,512).to(device)
    # input0=torch.randn(3,3,224,224).to(device)
    # # Flops, params = profile(model.module.to(device), input0) # macs
    # # print('Flops: % .4fG'%(Flops / 1e6))# 计算量
    # # print('params参数量: % .4fM'% (params / 1e6))
    # flops = FlopCountAnalysis(model.module.to(device), input0)
    # print(f"总计算量: {flops.total() / 1e9:.2f} GFLOPs")  # 转换为 GFLOPs（1e9）
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        print('args.gpu,args.gpu_map:',args.gpu,args.gpu_map) """

        if args.gpu is not None:
            input = input.cuda()
            model=model.cuda()
            #Flops, params = profile(model.module, inputs=(input,)) # macs
        target = target.cuda()
...
def validate(val_loader, model, criterion, args, printer, start_time = None):
    ...
    #compute model parameters
    total_params = sum(p.numel() for p in model.parameters())
    print(f"ResNet-50总参数量：{total_params/1e6} M")
